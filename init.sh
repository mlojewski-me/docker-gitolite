...
if [ ! -f ./.ssh/id_rsa ]; then
   ssh-keygen -f /home/git/.ssh/id_rsa -b 4096 -N ''
...
if [ ! -d ./.gitolite ] ; then
   ...
   if [ -n "$SSH_KEY" ]; then
       ...
       USER=git gitolite setup -a dummy
       ...
       cd /home/git/repositories/gitolite-admin.git && GL_LIBDIR=$(gitolite query-rc GL_LIBDIR) hooks/post-update refs/heads/master
   fi
else
...
if [ "${1}" = 'sshd' ]; then
  set -- /usr/sbin/sshd -D -f /etc/sshd_config
fi
